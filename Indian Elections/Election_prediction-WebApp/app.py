...
def main():
    if vote is None:
        st.error("Dataset load nahi hua. Please CSV file check karein.")
        return
    ...
    if choice == "About":
        ...
        st.image("images.jpeg", caption="Indian Election Analysis", use_container_width=True)
    ...
    # Visualizations section
        ...
        # Plot the heatmap with explicit figure pass
    ...
    # Prediction Section
        ...
        vote_model = vote.copy()
        le = LabelEncoder()
        vote_model['GENDER'] = le.fit_transform(vote_model['GENDER'])
        vote_model['PARTY'] = le.fit_transform(vote_model['PARTY'])

        features = ['GENDER', 'CRIMINAL\nCASES', 'AGE', 'ASSETS', 'LIABILITIES']
        X = vote_model[features]
        y = vote_model['WINNER']
    ...
    # Overview section
        ...
        st.write("""The dataset contains information on Indian election constituencies, including details about candidates, parties, and electoral statistics...""")
        st.markdown("---")
